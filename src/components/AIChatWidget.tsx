    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50 group">
        ...
        <button
          ...
          className="relative w-14 h-14 bg-gradient-to-br from-accent to-accent-hover rounded-full flex items-center justify-center text-accent-contrast shadow-lg shadow-accent/40 hover:shadow-xl hover:shadow-accent/50 hover:scale-110 active:scale-95 transition-all duration-300"
        >
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-[360px] sm:w-[420px] h-[560px] max-h-[80vh] bg-app/95 backdrop-blur-xl border border-theme rounded-2xl shadow-[0_8px_64px_rgba(0,0,0,0.35)] flex flex-col z-50 overflow-hidden animate-fade-up">
          {/* Header */}
          <div className="px-5 py-4 bg-gradient-to-r from-surface/80 to-surface/40 border-b border-theme flex items-center gap-3">
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5" style={{ WebkitOverflowScrolling: 'touch' }}>
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 ${msg.role === 'user' ? 'bg-accent text-accent-contrast rounded-tr-sm' : 'bg-elevated/80 text-muted rounded-tl-sm border border-theme/50'}`}>
                  {msg.role === 'assistant' ? (
          ...
          {/* Input Area */}
          <form onSubmit={handleSubmit} className="px-4 py-3.5 border-t border-theme/80 bg-surface/40">
            <div className="relative flex items-center gap-2">
              <input
                ...
                className="flex-1 bg-elevated/60 border border-theme rounded-xl px-4 py-2.5 text-sm text-primary placeholder-subtle focus:outline-none focus:border-accent/40 focus:bg-elevated/80 transition-all duration-200"
                disabled={isLoading}
              />
              <button
                type="submit"
